...
class _AuthState:
    ...
    async def _validate(self):
        ...
        if not self._hasattrs("access_token", "user_id"):
            ...
            jar.save(self._twitch.modified_cookie_path) # modified by Hitokage
        self._logged_in.set()
...
class Twitch:
    def __init__(self, settings: Settings):
        ...
        # modified by Hitokage
        # 重写cookie路径 接受传参
        from pathlib import Path
        self.modified_cookie_path = Path(r"D:\南ことり\Tools\Twitch Drops Miner\workflow_2024_04_09", f"slht{settings.num}", "cookies.jar")
    ...
    async def get_session(self) -> aiohttp.ClientSession:
        ...
        try:
            # modified by Hitokage Edit jar path
            print(self.modified_cookie_path)
            if self.modified_cookie_path.exists():
                cookie_jar.load(self.modified_cookie_path)
        except Exception:
    ...
    async def shutdown(self) -> None:
        ...
        if self._session is not None:
            ...
            cookie_jar.save(self.modified_cookie_path) # modified by Hitokage
    ...
    async def fetch_inventory(self) -> None:
        ...
        for campaign in available_list.copy(): # modified 优化性能
            if not any(kwd in campaign["name"] for kwd in DROP_NAME_KEYWORD): available_list.remove(campaign)
